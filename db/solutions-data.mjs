...
• Prüfungs-Zeitmanagement: zuerst den lauffähigen Kern (LEDs + kommentierter Code) sichern, Feinheiten wie die Blink-Phase danach.`,
  ...
  grundlagen: {
    "was-ist-ein-arduino": {
      didactics: `• Häufigste Fehlvorstellung: "Arduino = kleiner Computer." Besser so erklären: Ein Arduino ist ein Mikrocontroller, der genau ein Programm dauerhaft ausführt, kein Betriebssystem, keine Multitasking-Fähigkeit. Die Vergleichstabelle Computer vs. Arduino an der Tafel gemeinsam füllen lassen.
...
• Wenn ein echtes Board vorhanden ist: herumgehen lassen, Größe (Kreditkarte) und den ATmega-Chip als "Gehirn" zeigen — macht den Unterschied zum PC sofort begreifbar.`,
    ...
    "das-arduino-uno-board": {
      didactics: `• Mit einem echten Uno (oder dem SVG-Board) starten und die Bauteile zeigen lassen, bevor abstrakt geredet wird: USB-Buchse, Strombuchse, ATmega328P, Reset-Knopf, eingebaute LED an Pin 13, Power-LED.
...
• Prüfungsbezug: Anzahl und Bezeichnung der Pins (14 digital 0-13, 6 analog A0-A5), Pin-Typ-Zuordnung zu Bauteilen und die Rolle von GND/5V werden abgefragt. "14 digital / 6 analog" als Faktenwissen festhalten.`,
    ...
    "strom-spannung-und-widerstand": {
      didactics: `• Häufigste Fehlvorstellung: "Spannung fließt" bzw. Strom und Spannung verwechselt. Besser mit der Wasser-Analogie: Spannung (U) = Druck, Strom (I) = fließende Wassermenge pro Zeit, Widerstand (R) = enge Stelle im Rohr. Es fließt der Strom, die Spannung treibt an.
...
• LED-Polung praktisch zeigen: langes Bein = Anode (+, Richtung Pin/Widerstand), kurzes Bein = Kathode (-, Richtung GND). Merksatz für die Fehlersuche: "LED leuchtet nicht? Erst einmal umdrehen." Falsch herum geht sie meist nicht kaputt, sie sperrt nur.`,
    ...
    "die-arduino-ide-und-dein-erstes-programm": {
      didactics: `• Den Ablauf Schreiben -> Überprüfen (Verify/Kompilieren) -> Hochladen (Upload) sauber trennen: Verify prüft nur auf Fehler und übersetzt in Maschinensprache (kompilieren), Upload schickt das Ergebnis erst aufs Board. Begriffe Sketch, kompilieren, hochladen, Port/Board-Auswahl einführen.
...
• Prüfungsbezug: Die Struktur eines Sketches (Begriff Sketch, setup() einmal / loop() endlos, Semikolon-Regel, Sinn von Variablen und Kommentaren) und das Lesen/Ordnen eines Blink-ähnlichen Programms sind typische BW-Aufgaben.`,
  ...
  digital: {
    "leds-ansteuern": {
      sketch: `// LED an Pin 8 ansteuern (Musterlösung: blinken im Sekundentakt)
...
// Variante "nur leuchten" (Praxis-Aufgabe): loop() enthält nur digitalWrite(ledPin, HIGH);`,
      wiring: `LED an Pin 8: Vom Pin 8 geht ein Jumper zum 220-Ohm-Vorwiderstand. Der Widerstand liegt in Reihe zwischen Pin und LED-Anode — die Reihenfolge (Pin -> Widerstand -> Anode oder Pin -> Anode -> Widerstand -> GND) ist egal, Hauptsache der Widerstand sitzt im Stromkreis.
...
• Erst Hardware sauber aufbauen und mit der Dauer-AN-Variante testen (loop nur digitalWrite HIGH), dann das Blinken ergänzen — so trennt man Verdrahtungsfehler von Code-Fehlern.`,
    ...
    "wechselblinker": {
      sketch: `// Wechselblinker: zwei LEDs leuchten abwechselnd (wie ein Bahnübergang)
...
void loop() {
  ...
  delay(1000);             // 1 Sekunde halten
      ...
      wiring: `Zwei identisch aufgebaute LED-Stränge: LED 1 (grün) an Pin 12, LED 2 (rot) an Pin 13.
...
Beide Kathoden gehen auf dieselbe gemeinsame Minus-/GND-Schiene des Breadboards; diese Schiene per Jumper an einen GND-Pin des Arduino (ein gemeinsamer GND reicht für beide LEDs).`,
      mistakes: `• Nur ein gemeinsamer Widerstand für beide LEDs: jede LED braucht ihren eigenen 220 Ohm, sonst leuchten sie unterschiedlich hell oder beeinflussen sich.
...
• Sehr kleines delay (< 100 ms) als Stroboskop: im Unterricht bei >= 200 ms bleiben.`,
      didactics: `• Prüfungsbezug: Der Sketch zeigt, dass mehrere Pins unabhängig mit digitalWrite() gesteuert werden — genau dieses parallele Schalten mehrerer Ausgänge ist Grundlage für Ampel- und Lauflicht-Aufgaben der Abschlussprüfung.
...
• Modular testen: zuerst beide LEDs einzeln mit Dauer-AN prüfen, dann den Wechsel-Code laden — und über das delay() (1000/500/250) das Tempo bewusst variieren lassen.`,
    ...
    "led-lauflicht": {
      sketch: `// LED-Lauflicht: 5 LEDs leuchten nacheinander auf (Knight Rider, hin und zurück)
...
// auf wenige Zeilen — egal ob 5 oder 50 LEDs.`,
      wiring: `Fünf identisch aufgebaute LED-Stränge an Pin 8, 9, 10, 11, 12 (am besten in fünf verschiedenen Farben, das macht die Laufrichtung sichtbar).
...
• wartezeit als eine Variable nutzen lassen (statt fünf einzelne delay-Zahlen): zeigt den Wert von benannten Konstanten — eine Stelle ändern, alles wird schneller/langsamer. Tempo 200 -> 50 ms ausprobieren.`,
    ...
    "taster-als-eingabe": {
      sketch: `// Taster als Eingabe: Zustand lesen und im Serial Monitor anzeigen
...
void loop() {
  ...
  delay(100);                        // kurze Pause, damit der Monitor nicht überflutet wird
      ...
      wiring: `Taster über die Mittelrinne des Breadboards stecken, sodass die zwei Beinpaare auf gegenüberliegenden Hälften liegen (so sind die intern verbundenen Beine getrennt).
...
Logik merken: nicht gedrückt -> HIGH (1), gedrückt -> LOW (0), weil der Taster den Pin im gedrückten Zustand mit GND verbindet.`,
      mistakes: `• Taster nicht über die Mittelrinne, sondern längs gesteckt: die beiden gewählten Beine sind intern dauerhaft verbunden -> Pin liegt fest auf GND, scheint "immer gedrückt".
...
• Falsche Tasterbeine erwischt: die beiden benutzten Beine gehören zur selben internen Brücke -> Taster scheint wirkungslos.`,
      didactics: `• Prüfungsbezug: digitalRead(pin) und INPUT_PULLUP sind die Eingabe-Grundbausteine der BW-Abschlussprüfung — Voraussetzung für jede taster-/sensorgesteuerte Aufgabe. Die invertierte Logik (gedrückt = LOW) bewusst thematisieren.
...
• Floating-Problem mit der Feder-Analogie (Schalter ohne Rückstellung = zufällig) erklären und an der Tafel eine kleine Wahrheitstabelle (Taster | Pin-Zustand | Wert) anlegen. Das Prellen (delay(50)) erst hier kurz ankündigen, vertiefen in der nächsten Lektion.`,
    ...
    "led-mit-taster-steuern": {
      sketch: `// LED mit Taster steuern, Version 1: solange gedrückt = LED an (if/else)
...
• Stufenweise: erst Version 1 (gedrückt = an) stabil zum Laufen bringen, dann Version 2 (Toggle). Die Zustandstabelle (Taster jetzt | Taster vorher | Aktion) hilft, die Flankenerkennung zu verstehen — und das delay(50)-Entprellen durch Weglassen einmal bewusst "kaputt" zeigen.`,
    ...
    "einfache-ampelschaltung": {
      sketch: `// Einfache Ampelschaltung: deutsche Phasenfolge Rot -> Rot-Gelb -> Grün -> Gelb
...
*/`,
      wiring: `Drei LEDs wie eine echte Ampel anordnen: Rot oben (Pin 2), Gelb Mitte (Pin 3), Grün unten (Pin 4).
Pro LED: Pin -> eigener 220-Ohm-Vorwiderstand -> Anode (langes Bein) -> Kathode (kurzes Bein) -> gemeinsame Minus-/GND-Schiene.
Alle drei Kathoden auf dieselbe GND-Schiene; diese einmal per Jumper an einen GND-Pin des Arduino. Drei Vorwiderstände, einer pro LED.`,
      mistakes: `• Ein gemeinsamer Vorwiderstand für alle drei LEDs: ungleiche Helligkeit; jede LED braucht ihren eigenen 220 Ohm.
...
• Die Hilfsfunktion ampelSchalten() als didaktischen Höhepunkt nutzen: erst die lange, wiederholende Version verstehen, dann die kompakte Funktions-Version — das motiviert eigene Funktionen (Modularität) und bereitet auf wartbaren Code in der Prüfung vor.`,
  ...
  analog: {
    "spannungsteiler-verstehen": {
      wiring: `Fester Demo-Spannungsteiler als Reihenschaltung zwischen 5V und GND: 5V -> R1 (10-kOhm-Festwiderstand, oben) -> Mittelknoten (Abgriff U2) -> R2 (10-kOhm-Festwiderstand, unten) -> GND. Gemessen wird mit dem Multimeter (Gleichspannung, V=) zwischen dem Mittelknoten und GND: schwarze Messleitung an GND, rote an den Knoten zwischen R1 und R2. Erwartung bei zwei gleichen 10-kOhm-Widerständen: U2 ca. 2,5 V (die 5 V werden halbiert). Variante R2 = 20 kOhm -> U2 = 5 V * 20/(10+20) ca. 3,33 V; R1 = 20 kOhm (R2 = 10 kOhm) -> U2 ca. 1,67 V. Merkregel: U2 fällt immer über dem unteren Widerstand R2 ab. Der Arduino dient hier nur als 5-V-Quelle (USB anstecken). Dieser Mittelknoten ist genau der Punkt, an den in allen Folgelektionen A0 angeschlossen wird; statt eines festen R2 sitzt dort später der Sensor (LDR/NTC).`,
      mistakes: `• R1 und R2 vertauscht (oben/unten): Bei zwei gleichen 10-kOhm-Widerständen fällt es nicht auf (immer 2,5 V) — erst im Vergleichsversuch mit 20 kOhm zeigt sich der Fehler.
...
• Arduino nicht per USB versorgt -> kein Strom -> Multimeter zeigt 0 V.`,
      didactics: `• Bewusst machen: Dies ist die rechnerische Vorstufe ohne Code. Das Ergebnis U2 wird erst in der nächsten Lektion über analogRead() (0-1023) in eine Zahl gewandelt — Brücke explizit benennen: 2,5 V -> ca. 512, 5 V -> 1023, 0 V -> 0.
...
• Den Spezialfall "gleiche Widerstände -> halbe Spannung" als Schätzfrage drillen — kommt in der Prüfung häufig vor. Das Multimeter ist hier das Diagnose-Werkzeug, das später der Serial Monitor übernimmt.`,
    ...
    "analoge-eingaenge": {
      sketch: `// Potentiometer auslesen und im Serial Monitor anzeigen
...
• Baudrate im Serial Monitor (z. B. 115200) passt nicht zu Serial.begin(9600) -> Kauderwelsch.`,
      didactics: `• Zentrale Prüfungsunterscheidung verankern: analog = analogRead(), Pins A0-A5, Wertebereich 0-1023 (10-Bit-Wandler) — digital = digitalRead(), Pins 0-13, nur HIGH/LOW. Analogie Dimmer vs. Lichtschalter.
...
• Serial Monitor als Diagnose-Werkzeug: erst die echten Zahlen beim Drehen beobachten, bevor in der Folgelektion map() und Schwellenwerte darauf aufbauen. 512 als "Mitte = halbe Spannung" mit dem Spannungsteiler-Spezialfall der Vorlektion verknüpfen.`,
    ...
    "pwm-dimmen-statt-schalten": {
      sketch: `// LED-Helligkeit stufenlos mit Potentiometer steuern (PWM)
...
• Argument-Reihenfolge von map() durcheinander: map(wert, vonMin, vonMax, zuMin, zuMax) — Quell- vor Zielbereich.`,
      didactics: `• Den Doppel-Charakter klarmachen: analogRead() (Eingang, 0-1023, 10 Bit) vs. analogWrite() (Ausgang, 0-255, 8 Bit, nur PWM-Pins). PWM ist kein echtes Analog-Signal, sondern schnelles Ein-/Ausschalten (ca. 490 Hz) -> Auge nimmt Durchschnitt wahr.
...
• Serial Monitor parallel zur LED laufen lassen: Schüler sehen Poti-Wert und LED-Wert gleichzeitig — macht die Kette Eingang->Verarbeitung->Ausgang greifbar. Brücke zur Prüfungsschaltung: dieselbe Kette LDR/NTC -> map -> Aktor steuert später Dimmen statt hartem Schalten.`,
    ...
    "lichtsensor-ldr": {
      sketch: `// Lichtsensor (LDR) auslesen und im Serial Monitor anzeigen
...
• digitalRead() statt analogRead() -> nur HIGH/LOW, der feine Helligkeitsverlauf geht verloren.`,
      didactics: `• Bewusst machen: A0 reagiert auf die Spannung am Knoten — nicht direkt auf den Widerstand. Der Arduino kann keinen Widerstand messen, nur Spannung; deshalb der Spannungsteiler-Trick (Rückbezug zur Spannungsteiler-Lektion).
...
• Diese Lektion ist die direkte Vorstufe zur Prüfungsschaltung (LDR-Nachtabschaltung): hier nur Sensor auslesen, das Schalten der LED per if folgt in "Entscheidungen mit Sensorwerten".`,
    ...
    "ntc-temperatursensor": {
      sketch: `// NTC-Temperatursensor auslesen und im Serial Monitor beobachten
...
void loop() {
  ...
  delay(500);                   // halbe Sekunde Pause
      ...
      wiring: `NTC-Spannungsteiler: 5V -> 10-kOhm-Festwiderstand (R1, oben) -> A0-Knoten -> NTC (R2, unten) -> GND; A0 greift am Knoten zwischen Festwiderstand und NTC ab. Auf die Reihenfolge achten: NTC sitzt unten gegen GND, der feste 10 kOhm oben gegen 5V. Bei dieser Anordnung gilt: kalt -> NTC-Widerstand hoch -> hoher A0-Wert (0 °C ca. 786); warm/heiß -> NTC-Widerstand niedrig -> niedriger A0-Wert (Zimmer 25 °C ca. 511, 50 °C ca. 270). Die NTC-Beine sind nicht gepolt. USB anstecken, Serial Monitor 9600 Baud öffnen, dann Finger auf den NTC legen -> Wert sinkt; loslassen -> Wert steigt nach einigen Sekunden zurück (thermische Trägheit ist normal).`,
      mistakes: `• NTC und 10-kOhm-Festwiderstand vertauscht (NTC oben statt unten) -> Temperatur-Logik kehrt sich um: warm würde dann hoher Wert ergeben.
...
• Merksatz falsch herum gemerkt: heiß = KLEINER Zahlenwert (nicht größer).`,
      didactics: `• Kernmerksatz drillen: "Heißer Sensor, kleiner Zahlenwert." Ursache über den Spannungsteiler herleiten (NTC unten, R sinkt -> U2 sinkt -> analogRead sinkt) — nicht auswendig, sondern aus dem Prinzip der Vorlektion ableiten.
...
• Serial Monitor als Kalibrier-Werkzeug: erst Werte bei Zimmer- und Fingerwärme ablesen, dann in der Folgelektion mit if einen Schwellenwert darauf aufsetzen. Diese Kalibrier-Disziplin ist prüfungsrelevant.`,
    ...
    "entscheidungen-mit-sensorwerten": {
      sketch: `// Automatisches Nachtlicht mit Hysterese (zwei Schwellenwerte)
...
• analogRead (Sensor, 0-1023) und digitalWrite (LED, HIGH/LOW) verwechselt — Eingang lesen vs. Ausgang schreiben.`,
      didactics: `• Hier schließt sich die Kette: analoger Eingang (analogRead, 0-1023) trifft per if/else if/else eine Entscheidung und steuert einen digitalen Ausgang (digitalWrite, HIGH/LOW). Diese Trennung analog-rein/digital-raus ist prüfungszentral.
...
• Workflow festklopfen: erst Hardware bauen -> dann mit Serial Monitor echte Werte messen -> dann Schwellen festlegen -> zuletzt if-Logik schreiben. Diese Reihenfolge (Messen vor Programmieren) ist die Kernkompetenz für die RSAP-Sensor-Aufgaben. Diese Lektion ist die direkte Vollausbaustufe der LDR-Nachtabschaltung.`,
  ...
  aktoren: {
    "servomotor-ansteuern": {
      sketch: `// Servomotor ansteuern -- SG90-Servo bewegt sich endlos hin und her
...
// Die for-Schleife fährt alle Zwischenwinkel an -> flüssige Bewegung statt Sprung.`,
      wiring: `SG90-Servo hat ein dreiadriges Kabel. Braun (oder schwarz) = Masse an einen GND-Pin des Arduino. Rot = Versorgung an 5V. Orange (oder gelb) = Signal an Pin 9. Die kleinen SG90 dürfen direkt am Arduino-5V hängen (USB-Strom reicht für einen Servo). Merkhilfe: das außenliegende Kabel ist immer das Signal, die Mitte immer Plus (rot). Über das Breadboard: rotes Servokabel auf die +Schiene, braunes auf die -Schiene, oranges zu Pin 9. Größere Servos oder mehrere Servos brauchen eine externe 5-V-Versorgung; deren Masse muss zwingend mit dem Arduino-GND verbunden werden (gemeinsame Masse). Niemals Plus (rot) mit dem Signal-Pin verwechseln — das kann den Arduino-Pin durchbrennen.`,
      mistakes: `• Plus (rot) und Masse (braun) vertauscht -> Servo überhitzt, im schlimmsten Fall stirbt auch der Arduino-Pin.
...
• Winkel > 180 in write() geschrieben -> Standard-SG90 fährt nur 0 bis 180 Grad.`,
      didactics: `• Prüfungskern "Aktor ansteuern": Der Servo ist der Einstieg in bewegte Aktoren. Hier die saubere Pin-Zuordnung 5V/GND/Signal einüben — in RSAP-Pool-Aufgaben (Gewächshaus-Belüftung 0°/90°, Scheibenwischer 30°-150°, Kurvenlicht 60°-120°) wird dieses Grundgerüst nur um einen Sensor (NTC/Poti/Schalter) erweitert.
...
• Konzept-Bruch benennen: Erstmals eine Library (#include), eine eigene Bauteil-Variable (Servo meinServo;) und neue Methoden (attach()/write()). attach() übernimmt das pinMode() automatisch — gut zum Kontrastieren mit der digitalWrite()-Welt des Transistors.`,
    ...
    "transistor-als-schalter-grundlagen": {
      sketch: `// Transistor als Schalter -- DC-Motor 2 s an, 2 s aus
...
• Software-Sicht entkoppelt von Hardware: Der Code ist identisch zur LED-Ansteuerung (digitalWrite HIGH/LOW). PWM hier nur als Plus-Box (analogWrite für Drehzahl). Wichtige Klarstellung: Laut BW-Skript wird der DC-Motor selbst mit dem L298N gesteuert, nicht mit Einzeltransistor; diese Lektion ist die Grundlage.`,
    ...
    "dc-motor-mit-l298n": {
      sketch: `// DC-Motor mit L298N -- Richtung (IN1/IN2) + Drehzahl (ENA per PWM)
