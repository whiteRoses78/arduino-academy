...
## 2026-06-07 — Kompetenztest-Ausbau: Modul 1 (Grundlagen) ✅ LIVE

- **Was:** Alle 5 Grundlagen-Lektionen haben jetzt einen Kompetenztest (je 6 MC-Fragen, 30 gesamt): „Was ist ein Arduino?", „Das Arduino Uno Board", „Strom, Spannung & Widerstand", „Die Arduino IDE & dein erstes Programm", „setup() und loop()". Damit ist Modul 1 als erstes Modul testseitig komplett (Rollout-Strategie: Modul für Modul).
- **Fragen-Policy (Marco, dauerhaft):** Modul 1 = 6 Fragen/Lektion, **ab Modul 2 mindestens 8**. Gleiche Anzahl je Lektion innerhalb eines Moduls → vergleichbare Prozente.
...
## 2026-06-07 — Kompetenztest-Ausbau: Modul 3 (Analog) ✅ LIVE

- **Was:** Alle 6 Analog-Lektionen haben jetzt einen Kompetenztest (je **8 Fragen**, Policy ab Modul 2 ≥ 8): Spannungsteiler verstehen, Analoge Eingänge, PWM (Dimmen statt Schalten), Lichtsensor (LDR), NTC-Temperatursensor, Entscheidungen mit Sensorwerten. Damit ist Modul 3 testseitig komplett — **48 neue Fragen** (Modul 3 hat als einziges 6 statt 5 Lektionen).
- **Wie:** Gleiche Mechanik wie Modul 1/2 — Fragen in `db/test-questions-data.mjs` (re-seed-feste Quelle, Key `<modul>/<slug>`), via `node db/gen-test-sql.mjs analog` erzeugt und per MCP `execute_sql` (idempotentes delete+insert) in die Prod-DB eingespielt. **Kein neuer Code**: Das Frontend zeigt den Test-Block automatisch, sobald `has_test(lesson_id)` true ist → mit dem DB-Insert **sofort live** (darum erst Marco-Freigabe, dann Einspielen).
- **Erstellung:** 6 parallele Subagenten (1/Lektion) entwarfen die Fragen aus dem Supabase-Lektionsinhalt → neuer wiederverwendbarer Generator `db/gen-review.mjs` (rendert das Review aus **derselben** Quelle, die eingespielt wird → kein Auseinanderdriften) → Review-Doc `docs/review-test-analog.md` → Fakten-Check (Claude) → Marco-Freigabe („passt, spiel ein").
- **Fakten-Check-Schwerpunkt (die typische Fehlerquelle):** Sensor-Spannungsteiler gegen die echte Lektionsquelle geprüft — LDR sitzt **oben** (5V→LDR→A0→10kΩ→GND) ⇒ hell = hoher Wert (~920); NTC sitzt **unten** (R2 an GND) ⇒ heiß = niedriger Wert; Spannungsteiler-Rechnungen (10k/20k→3,33V; NTC 10k→4k→1,43V) und Kennlinie (25°C→10kΩ→2,5V→~511) bestätigt. Die zwei unterschiedlichen Topologien sind kein Fehler, sondern stehen so in den Lektionen.
- **Verifiziert:** je Lektion 8 Fragen + `has_test=true`; **alle 48 `correct`-Indizes zeigen exakt auf die richtige Antwort** (Antwort-Positionen verteilt, kein „immer A"); Sonderzeichen-Round-Trip (Ω, ·, ≈, ~, Umlaute korrekt in der DB); Live-Check (Anon: Block + Anmelde-Hinweis, kein „Test starten", kein Leak — test-exklusive Distraktoren nicht im Anon-HTML). Spec/Quelle: `db/test-questions-data.mjs`, `docs/review-test-analog.md`.

---

## 2026-06-07 — Kompetenztest-Ausbau: Modul 4 (Aktoren) ✅ LIVE

- **Was:** Alle 3 Aktoren-Lektionen haben jetzt einen Kompetenztest (je **8 Fragen**, 24 gesamt): Servomotor ansteuern, Transistor als Schalter (Grundlagen), DC-Motor mit L298N. Aktoren ist das hardware-lastigste Modul (Servo-Winkel, Transistor-Pinbelegung, L298N-Pins) — darum diesmal mit verschärftem Review.
- **Zwei-stufiger Review (Marco-Vorgabe „beim Reviewer extra genau hinschauen"):** (1) 3 Entwurfs-Subagenten je Lektion aus dem Supabase-Lektionsinhalt; (2) **3 unabhängige Prüf-Subagenten** (haben die Fragen NICHT geschrieben) zogen den Inhalt frisch und checkten jede Frage adversarial gegen die Quelle — Pflicht: wörtliches Beleg-Zitat je Frage. Ergebnis: **24/24 „OK" mit Beleg**, 0 Probleme. Bestätigte Kernfakten: BC547 C-B-E (flache Seite zum Betrachter), 1N4148-Freilaufdiode Kathode/Ring zur +5V, Arduino-Pin 20 mA vs. Motor 50-100 mA, L298N ENA=Pin 10/IN1=9/IN2=8 + Drehrichtungs-/Stopp-Logik, Servo `write(90)`=Mitte + Kabelfarben, mehrere Troubleshooting-Symptome stammen wörtlich aus `haeufige_fehler` der Lektionen.
- **Wie:** Gleiche Mechanik wie Modul 1-3 — Fragen in `db/test-questions-data.mjs`, via `node db/gen-test-sql.mjs aktoren` + MCP `execute_sql` (idempotentes delete+insert). **Kein neuer Code** (Block über `has_test`). Zusätzlich Antwort-Positionen exakt auf A/B/C/D verteilt (`01230123` pro Lektion = 2/2/2/2, nicht erratbar) — beim Servo lagen vorher 5/8 auf Position B.
- **Verifiziert:** je Lektion 8 Fragen + `has_test=true`; **alle 24 `correct`-Indizes zeigen exakt auf die richtige Antwort**; Zeichen sauber; Live-Check (Anon: Block + Anmelde-Hinweis, kein „Test starten", kein Leak). Review-Doc `docs/review-test-aktoren.md`. **Stand Test-Rollout: 4 von 5 Modulen fertig (Grundlagen, Digital, Analog, Aktoren); offen: Modul 5 = projekt (3 Lektionen).**

---

## 2026-06-07 — Kompetenztest-Ausbau: Modul 5 (Projekt) ✅ LIVE — ROLLOUT KOMPLETT

- **Was:** Die 3 Projekt-Lektionen haben jetzt einen Kompetenztest (je **8 Fragen**, 24 gesamt): Ampel mit Fußgängerüberweg, Nachtabschaltung mit Lichtsensor, Prüfungsschaltung komplett. **Damit hat der Kompetenztest jede der 23 Lektionen abgedeckt — Rollout über ALLE 5 Module abgeschlossen (174 Fragen gesamt: grundlagen 30, digital 48, analog 48, aktoren 24, projekt 24).**
- **Zwei-stufiger Review (wie Modul 4):** 3 Entwurfs-Subagenten + 3 unabhängige Prüf-Subagenten (adversarial, Beleg-Zitat-Pflicht) → **24/24 „OK"**. Geprüft: Ampel-Pins (autoGruen=4, Taster Pin 7/INPUT_PULLUP) + Phasenfolge/Zeiten (Gelb 2s, Räumzeit 1s, Fußgänger-Grün 5s, 3× blinken, Rot-Gelb 1s); LDR-Schaltung 5V→LDR→A0→10kΩ→GND + hell=hoher Wert + SCHWELLE=300 (dunkel→Ampel aktiv); RSAP-Regel (1 Sensor + 2 Aktoren ODER 2 Sensoren + 1 Aktor, ≥1 analog), Aufbau-Reihenfolge, „GND vergessen" aus der Fehlerliste. Konsistenz über die 3 Lektionen bestätigt (gleiche Pins/Schaltung, da pruefungsschaltung = Ampel + Nachtabschaltung zusammengeführt).
- **Wie:** Gleiche Mechanik (Fragen in `db/test-questions-data.mjs`, `node db/gen-test-sql.mjs projekt` + MCP `execute_sql`). Kein neuer Code. Antwort-Positionen `01230123` pro Lektion.
- **Verifiziert:** je Lektion 8 Fragen + `has_test=true`; **alle 24 `correct` exakt richtig**; Gesamt-DB-Check: 23/23 Lektionen mit Test, 174 Fragen, 0 kaputte `correct`; Zeichen sauber; Live-Check (Anon: Block + Anmelde-Hinweis, kein Leak). Review-Doc `docs/review-test-projekt.md`.

---

## 2026-06-10 — Test-Overlay (kein Nachlesen) + Registrierung ausgeblendet ✅ LIVE

- **Was:** Während ein Kompetenztest läuft, liegen die Fragen jetzt in einer Vollbild-Ansicht über der Seite — die Lektion ist nicht mehr parallel nachlesbar (Fund aus dem 1. Klasseneinsatz 08.06., von den Schülern selbst bemängelt). Oben im Overlay: Lektionstitel + Zähler „X von Y beantwortet". Kein Schließen-Knopf; raus geht es nur über „Abschicken" (Auswertung erscheint danach wie gewohnt in der Lektion). Außerdem: „Registrieren"-Knopf aus Header + Anmelde-Seite entfernt, /registrieren erklärt die geschlossene Testphase („Konten vergibt die Lehrkraft") — die Selbst-Registrierung ist seit 10.06. Supabase-seitig deaktiviert (disable_signup, verifiziert; bestehende Logins unberührt).
...
- **Verifiziert:** tsc/lint/28 Tests/build grün; eingeloggter Overlay-Check via puppeteer (Schüler-Konto, OHNE Abschicken — 0 Versuche verbraucht, per DB bestätigt): Overlay deckt Lektion+Header @375 und @1280, `body.overflow=hidden`, Zähler 0→1, Abschicken bis zur letzten Antwort gesperrt; Anon-Gegenprobe: Lektionsseite unverändert, Header ohne „Registrieren", /registrieren = Hinweis-Karte, /anmelden ohne Registrier-Link. Puppeteer-Lehre: `setViewport` mit `isMobile`-Wechsel lädt die Seite neu (React-State weg) — Mobile-Checks mit mobilem Viewport STARTEN. Spec `docs/superpowers/specs/2026-06-10-test-overlay-design.md`, Plan `docs/superpowers/plans/2026-06-10-test-overlay.md`.
- **Nebenfund (offen):** Etliche Testfragen in projekt/analog/aktoren enthalten ae/oe/ue statt echter Umlaute (z. B. „Pruefungsschaltung") — separater Content-Fix in `db/test-questions-data.mjs` + Re-Seed, braucht keinen Deploy.

---

## 2026-06-10 — Umlaut-Fix Testfragen (12 Lektionen, DB-only) ✅ LIVE

- **Was:** 376 falsche Umschreibungen in den Testfragen korrigiert: ae/oe/ue → ä/ö/ü (348), falsches ss → ß (38: Fußgänger, größer, heißt, fließt, außerhalb, Straßenlaterne …) plus Tippfehler „lasst"→„lässt" und Versal-Fall „UEBER"→„ÜBER". Betroffen: alle 6 analog-, alle 3 aktoren-, alle 3 projekt-Lektionen (12 von 23; grundlagen + digital waren sauber — der digital-Verdacht war `true` in Code-Optionen). Entdeckt beim Overlay-Check des Test-Features.
- **Wie (wortgenau, nicht pauschal):** Ersetzung nur über eine manuell geprüfte Wortliste aus den Frage-Feldern (162 Kandidaten klassifiziert). Geschützt blieben: Code-Bezeichner (`autoGruen`, `true`), korrekte oe/ue-Wörter (Temperaturkoeffizient, Quelle, Frequenz, zuerst, Steuer-/dauer-/bauen-Familie) und korrekte ss-Wörter (Abschluss, dass, muss, misst, Masse=GND). Lektions-Slugs (Keys) unangetastet.
- **Einspielen:** `db/test-questions-data.mjs` (Quelle) korrigiert → `gen-test-sql.mjs` je Modul → MCP `execute_sql` (idempotentes delete+insert), nur die 3 betroffenen Module. Kein Deploy nötig (reiner DB-Content).
- **Verifiziert:** Transport-Check Quelle↔DB zeichengenau identisch (12/12 Lektionen, je 8 Fragen); `correct`-Indizes/Optionszahl/Typen 0 Abweichungen (Quelle alt vs. neu verglichen); global 174 Fragen / 23 Lektionen / 0 kaputte Wörter / `autoGruen` vorhanden; 30 bestehende `test_attempts` unberührt (keine der 12 Lektionen hatte Versuche).
